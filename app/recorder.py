...
class SignalRecorder:
    ...
    def record_tick(self, tick, metrics, signal, fsm_state: str, profile=None, signal_quality=None, paper_trade_opened: bool = False, paper_trade_result: str | None = None, paper_trade=None, paper_stats=None) -> None:
        ...
        event = {
            ...
            "signal_group_id": int(getattr(signal_quality, "signal_cluster_id", 0)) if signal_quality else 0,
            "signal_cluster_id": int(getattr(signal_quality, "signal_cluster_id", 0)) if signal_quality else 0,
            "signal_grade": str(getattr(signal_quality, "grade", "")) if signal_quality else "",
            "signal_quality_grade": str(getattr(signal_quality, "grade", "")) if signal_quality else "",
            "signal_quality_score": float(getattr(signal_quality, "quality_score", 0.0)) if signal_quality else 0.0,
            "signal_quality_reasons": list(getattr(signal_quality, "reason_codes", [])) if signal_quality else [],
            "signal_quality_components": dict(getattr(signal_quality, "components", {})) if signal_quality else {},
            "is_new_market_event": bool(getattr(signal_quality, "is_new_market_event", False)) if signal_quality else False,
            "is_duplicate_signal": bool(getattr(signal_quality, "is_duplicate_signal", False)) if signal_quality else False,
            "post_1s_return_pct": 0.0,
            "post_3s_return_pct": 0.0,
            "post_5s_return_pct": 0.0,
            "post_10s_return_pct": 0.0,
            "max_favorable_10s_pct": 0.0,
            "max_adverse_10s_pct": 0.0,
